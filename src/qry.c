...
int qry_processar(const char* caminho) {
    ...
    while(fgets(linha, sizeof(linha), f)) {
        ...
        if(strcmp(cmd, "rq") == 0) {
            ...
            char cpf[CPF_TAM], cep[CEP_TAM], compl[COMPL_TAM], face[7], f;
            int num;
            
            if(sscanf(linha, "%*s %31s %31s %6s %d %15s", cpf, cep, face, &num, compl) == 5)
                f = face[strlen(face) - 1];
                switch(f) {
                    case 'W': f = 'O'; break;
                    case 'E': f = 'L'; break;
                }
                cmd_mud(cpf, cep, f, num, compl);
        } 
